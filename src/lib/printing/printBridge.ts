...
function printOnAllConnectedPrinters(order: Order, orderCode: string, config: PrintingConfig) {
  config.printers
    .filter(p => getPrinterStatus(p.id) === 'conectado')
    .forEach(p => printOrderOnPrinter(p.id, p.name, order, orderCode, config, p.paperWidth));
}

// "Recebido" — a brand-new order just landed (realtime INSERT).
export function handleOrderReceivedForPrinting(order: Order, orderCode: string, config: PrintingConfig | undefined) {
  if (!config?.autoPrintOnReceived) return;
  if (wasAutoPrinted(order.id, 'received')) return;
  markAutoPrinted(order.id, 'received');
  printOnAllConnectedPrinters(order, orderCode, config);
}

// "Confirmado" — staff accepted a pending order (received -> preparing).
export function handleOrderConfirmedForPrinting(order: Order, orderCode: string, config: PrintingConfig | undefined) {
